...
int fs_init_serial_driver(fs_device_info_t *device_handle)
{
        ...
        if (result == 0)
        {
            for (size_t i = 0; i < globbuf.gl_pathc; i++)
            {
                ...
                if (read_len > 0 && response[0] == 'I')
                {
                    ...
                    {
                        *end_char = '\0';
                        strncpy(device_handle->device_descriptor.device_name, response + 1, sizeof(device_handle->device_descriptor.device_name) - 1);
    ...
    else
    {
        ...
        char port_name[20];
        snprintf(port_name, sizeof(port_name), "/dev/ttyS%d", device_handle->device_params.serial_port);
        int fd = open(port_name, O_RDWR | O_NOCTTY | O_NONBLOCK);
        if (fd == -1)
        {
